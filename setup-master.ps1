# ====================================================================
# CYBER TERMINAL - ULTIMATE MASTER SUITE (100% COMPLETE & BUG-FREE)
# ====================================================================
Set-Location "C:\Users\gokul\cyber-terminal"
Write-Host "========================================================" -ForegroundColor Green
Write-Host " BUILDING COMPLETE MASTER CYBER TERMINAL (16 ENGINES)  " -ForegroundColor Green
Write-Host "========================================================" -ForegroundColor Green

# 1. WRITE src\utils.js
@'
...
Write-Host "[1/3] src\utils.js updated with 250k PBKDF2, Tokens, & Steganography." -ForegroundColor Green
...
@layer components {
  ...
  .cyber-btn-cyan { border-color: #00ffff; color: #00ffff; }
  .cyber-btn-cyan:hover:not(:disabled) { background: #00ffff; color: #000; box-shadow: 0 0 25px rgba(0,255,255,0.7); }
  .cyber-btn-red { border-color: #ff0040; color: #ff0040; }
  .cyber-btn-red:hover:not(:disabled) { background: #ff0040; color: #000; box-shadow: 0 0 25px rgba(255,0,64,0.7); }
  .scanlines {
...
Write-Host "[2/3] src\index.css updated." -ForegroundColor Green
...
function CoreAI() {
  ...
  const handleActivate = () => {
    ...
    setTimeout(() => {
      setStatus('ACCESS DENIED: KEY REVOKED OR UNPAID (VALUED AT ₹55,000/YR)');
  ...
  return (
    <div style={{ height:'100%', overflow:'auto', padding:'4px' }}>
      ...
      <div className="glow-box" style={{ padding:'20px', textAlign:'center', marginBottom:'14px' }}>
        ...
        <p style={{ fontSize:'17px', color:'#ff0040', fontWeight:'bold', marginBottom:'12px' }} className="glow-red">ACCESS VALUE: ₹55,000 / 1 FULL YEAR PASS</p>
...
function BitcoinEscrow() {
  ...
  const verifyKey = async () => {
    ...
    if (userKey.trim().toUpperCase() === valid) {
      setIsVip(true); localStorage.setItem('is_vip', '1');
      setStatusMsg('✓ KEY ACCEPTED: VIP GOD-MODE ENGAGED PERMANENTLY.');
      playBeep(1200, 250, 'sine');
    } else {
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', padding:'4px', overflow:'auto' }}>
      ...
      {isVip && (
        <div className="glow-box" style={{ padding:'12px', marginBottom:'12px', border:'1px solid #00ffaa' }}>
          <p style={{ color:'#00ffaa', fontWeight:'bold', fontSize:'12px' }}>★ VIP GOD-MODE ACTIVE: ALL MODULES PERMANENTLY UNLOCKED</p>
        </div>
      )}
      ...
      {showAdminConsole && (
        <div className="glow-box" style={{ padding:'16px', border:'1px solid #ff0040', background:'rgba(20,0,0,0.9)' }}>
          <p style={{ color:'#ff0040', fontWeight:'bold', fontSize:'12px', marginBottom:'6px' }}>⚡ SECRET ADMIN KEY MINTING CONSOLE (MASTER CODE 6712 VERIFIED)</p>
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', padding:'4px' }}>
      ...
      <div className="glow-box" style={{ flex:1, padding:'12px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
        ...
        {done && <p style={{ marginTop:'10px', fontSize:'11px', color:'#00FF00', fontWeight:'bold' }}>✓ VOLATILE RAM PURGED & ZEROED</p>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'#000', color:'#00FF00', fontFamily:"'Courier New', monospace", overflow:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:20, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'6px 12px', borderBottom:'1px solid #00FF00', background:'rgba(0,0,0,0.95)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
          <button onClick={() => { setSidebar(!sidebar); triggerHaptic([20]); }} style={{ background:'none', border:'1px solid #00FF00', color:'#00FF00', padding:'3px 7px', cursor:'pointer', fontFamily:'inherit' }}>☰</button>
          <div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'6px', padding:'3px', fontSize:'8px', background:'rgba(0,10,3,0.95)', borderBottom:'1px solid #002200', flexWrap:'wrap', zIndex:20 }}>
        ...
        <span>[YOU: 127.0.0.1]</span>➔<span>[GUARD: 🇩🇪 DE]</span>➔<span>[RELAY: 🇮🇸 IS]</span>➔<span>[EXIT: 🇨🇭 CH]</span>➔<span style={{ color:'#00ffaa', fontWeight:'bold' }}>[.ONION ACTIVE]</span>
      </div>
...
Write-Host "[3/3] src\App.jsx created with all 16 Hollywood Engines." -ForegroundColor Green

Write-Host ""
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host "  MASTER BUILD COMPLETED SUCCESSFULLY!                  " -ForegroundColor Cyan
Write-Host "========================================================" -ForegroundColor Cyan
